...
moderators = ["Javi_g18","Lucifer_Morningstars_","JoseRSG","Chaewon.24"]

cara = ["😢Cayó cruz, mejor suerte para la próxima…””, felicidades, ganaste, cayó cara… "]

cruz = ["😢cara, mejor suerte para la próxima...","felicidades, ganaste, cruz... "]
...
coco = ["su coco bello conmigo 🌽","usted cargo un tronco","su coco está molesto 😡"]

loca = ["🤪Tu Madness Livel es: 100%","🤪Tu Madness Livel es: 99%","🤪Tu Madness Livel es: 50%","🤪Tu Madness Livel es: 0% ","🤪Tu Madness Livel es : 1%","🤪Tu Madness Livel es: 2%","🤪Tu Madness Livel es: 64%","🤪Tu Madness Livel es: 22%","🤪Tu Madness Livel es: 19%","🤪Tu Madness Livel es: de: 88%", "🤪Tu Livel de la Locura es: 39%","🤪Tu Livel de la Locura es de : 40%","🤪Tu Livel de la locura es: 92%","🤪Tu Livel de la locura es: 74%","🤪Tu Livel de la locura es: 10%","🤪Tu Livel de la locura es: 9 %","🤪Tu Livel de la Locura es: 77%","🤪Tu Livel de la Locura es de: 82%","🤪Tu Madness Livel es: 66%","🤪Tu Madness Livel es: 11%","🤪Tu Madness Livel es: 15%"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(11 , 0 , 4 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} llego el ")  
        await self.highrise.send_whisper(user.id,f"{user.username} Bienvenid@ a la caverna del rock,pasala bien y ten un buen rato,para más info ve mi descripción")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith(("!cagar","/cagar")):
            ...
            await self.highrise.send_whisper(user.id,f"💩{user.username} comenzaste a cagar...")
            await asyncio.sleep(1)
            await self.highrise.send_whisper(user.id,f"💩está cagando...")
            await asyncio.sleep(1)
            await self.highrise.send_whisper(user.id,f"💩está cagando...")
            await asyncio.sleep(1)
            await self.highrise.send_whisper(user.id,f"💩está cagando...")
        ...
        if message.lower().startswith(("!balatro","/balatro")):
        ...
        if message.lower().startswith(("!moneda cruz","/moneda cruz")):
            ...
            await asyncio.sleep(2)
            await self.highrise.send_emote("emoji-flex",user.id)
            await self.highrise.send_whisper(user.id,f"{user.username} {random.choice(cruz)} 🪙")
        ...
        if message.lower().startswith("!tipall ") and user.username in moderators:
              ...
              try:
                  ...
                  await self.highrise.chat("💰Cantidad inválida")
                  return
              ...
              if bot_amount < total_tip_amount:
                  await self.highrise.chat("💰No hay fondos suficientes para dar regalo a todos")
                  return
              ...
              for room_user, pos in room_users.content:
                  ...
                  for bar in tip:
                      ...
                      await self.highrise.chat(f"💰Dio para {room_user.username} {amount} {bar}!")
        ...
        if message.lower().startswith("!tipme ") and user.username in moderators:
                try:
                    ...
                    if bot_amount < amount:
                        await self.highrise.chat("💰No hay dinero suficiente en mi cartera. ")
                        return
                    ...
                    if total > bot_amount:
                        await self.highrise.chat("💰No hay suficiente dinero para dar el valor pedido. ")
                        return
                    # Send tip to the user who issued the command
                    for bar in tip:
                        await self.highrise.tip_user(user.id, bar)
                    await self.highrise.chat(f"💰fuiste avisado {amount_str}.")
                except (IndexError, ValueError):
                    await self.highrise.chat("💰Valor de regalo inválido. Por favor, especifique un número válido. ")   
                    await self.highrise.chat(f"💰Dio para {user.username} {amount} {bar}!")
        ...
        if message.startswith("/dorado"):
           await self.highrise.react("clap",user.id)
           await self.highrise.send_whisper(user.id,"🟡Pesco 1x Pez Dorado🟡 USTED GANO LA MEDALLA : (MEGA PESCADOR)")
        ...
        if message.lower().startswith(("/bendage","!curative","!bendage","/curative")):
        ...
        if        message.startswith("Feo") or      message.startswith("feo") or      message.startswith("meado") or message.startswith("Meado"):
            await self.highrise.chat(f"RESPETA MMGV!!! {user.username} 🤬🤬")
            await self.highrise.send_emote("emote-swordfight")

        if        message.startswith("cuerno") or      message.startswith("Cuerno") or      message.startswith("Vagabundo") or message.startswith("vagabundo"):
            await self.highrise.chat(f"TU MAMA!!! {user.username} 🤬🤬")
            await self.highrise.send_emote("emote-swordfight")

        if        message.startswith("/personas") or      message.startswith("!personas"):
            room_users = (await self.highrise.get_room_users()).content
            await self.highrise.chat(f"Há  {len(room_users)} personas en la sala ")
            await self.highrise.send_emote("dance-floss")
                      
        if        message.startswith("buenote") or      message.startswith("buenote") or      message.startswith("buenote"):
            await self.highrise.send_emote("emote-blowkisses")
            await self.highrise.send_emote("idle-uwu", user.id)
            await self.highrise.chat(f"Tu tambien estas buenote(a) {user.username} 😳👉👈")

        if message.lower().lstrip().startswith(("emotes","!emotes","-emotes","/emotes")):
            await self.highrise.send_whisper(user.id, f"💡 Puedes usar emotes pagos gratis,Solo escribe su nombre en el chat. Ejemplo:\n\nswordfight\ngravity\nuwu\n\nPara ver todos los emotes gratuitos, use: !emote list")
        ...
        if message.lower().lstrip().startswith(("!help","/help","-help","help","/ayuda","!ayuda","ayuda","/ayuda")):
            await self.highrise.send_whisper(user.id, f"\n💡Me parece que estás buscando ayuda 🤓\n\n!mod\n!juegos\n!general") 
        ...
        if message.lower().lstrip().startswith(("!general","/general","-general","general")):
            await self.highrise.send_whisper(user.id, f"\n💡Acceso general gratuito para todos sin ningun problema..\n\n-emote @usuario!\n!emote\n!emote list\n!juegos\n!loop [emote|1-98]\n/Sin acceso\n!Sin acceso\n!userinfo @usuario\n!tele @usuario\n![react|heart|wink|wave|clap|thumbs]  @usuario 1-10")
            
        if        message.startswith("Lindo") or      message.startswith("LINDO") or      message.startswith("lindo"):
            await self.highrise.react("heart",user.id,)
            await self.highrise.chat(f"tu tambien eres lindo(a) {user.username} 🥰🥰")
            await self.highrise.send_emote("emote-shy",user.id)
            await self.highrise.send_emote("emote-blowkisses")
          
        if        message.startswith("Buen dia") or      message.startswith("Buen Dia") or      message.startswith("buen dia") or message.startswith("BUEN DIA"):
            await self.highrise.send_emote("emote-blowkisses")
            await self.highrise.send_whisper(user.id,f"Buen Dia {user.username} 😊🌅")

        if        message.startswith("Buena noche") or      message.startswith("buena noche") or      message.startswith("Buena Noche") or message.startswith("BUENA NOCHE"):
            await self.highrise.send_emote("emote-blowkisses")
            await self.highrise.send_whisper(user.id,f"Buena Noche {user.username} 😊🌃🌉")

        if        message.startswith("Buena tarde") or      message.startswith("buena tarde") or      message.startswith("Buena Tarde") or message.startswith("BUENA TARDE"):
            await self.highrise.send_emote("emote-blowkisses")
            await self.highrise.send_whisper(user.id,f"Buena Tarde {user.username} ☀️")
        ...
        if        message.startswith("🤣") or      message.startswith("😂") or             message.startswith("haha") or             message.startswith("Ha") or         message.startswith("Ka") or           message.startswith("kaka") or           message.startswith("ha") or          message.startswith("ks") or             message.startswith("kk") or             message.startswith("Kk") or message.startswith("😁") or message.startswith("😀"):
        ...
        if        message.startswith("/shy") or   message.startswith("shy") or      message.startswith("!shy") or      message.startswith("Shy") or message.startswith("13"):
            await self.highrise.send_emote("emote-shy",user.id)
            await self.highrise.chat(f" Disfruta de este emote {user.username} 😍")
        ...
        if        message.startswith("/hero") or      message.startswith("!hero") or      message.startswith("Hero") or    message.startswith("hero") or message.startswith("62"):
        ...
        if        message.startswith("/rest") or      message.startswith("!rest") or        message.startswith("rest") or    message.startswith("Rest") or message.startswith("98"):
        ...
        if              message.startswith("!emote all random") or                              message.startswith("/emote all random"):
          if user.username in moderators:
            ...
            for roomUser, _ in roomUsers:
                ...
                room_users = (await self.highrise.get_room_users()).content
            await self.highrise.chat(f"Enviando [Random Emotes] para {len(room_users)} personas.")

        if              message.startswith("Rest All") or                              message.startswith("/emote all jetpack") or       message.startswith("!emote all jetpack"):
          if user.username in moderators:
            roomUsers = (await self.highrise.get_room_users()).content
            for roomUser, _ in roomUsers:
                await self.highrise.send_emote("sit-idle-cute", roomUser.id)
                room_users = (await self.highrise.get_room_users()).content
            await self.highrise.chat(f"Enviando [Rest] para {len(room_users)} personas.")
        ...
        if message.startswith("/cartilla1"):
            await self.highrise.send_whisper(user.id,"aqui tiene nuestra lista de bebidas espero las disfrute y deje propina")
                                     
        if message.startswith("/carta1"):
            await self.highrise.send_whisper(user.id,"/glup , /polarcita , /malta , /vino , /vodka , /whisky , /ron , /mal-portada , /dragon / , /cerveza , /coca , /rumbaron , /agua , /anis , /5-estrellas , /macondo")

        if message.startswith("/glup"):  
            await self.highrise.send_whisper(user.id,f"{user.username} aqui está su glup🧊🥤 ")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/polarcita"):  
            await self.highrise.send_whisper(user.id,f"{user.username} aqui esta su polarcita🥃")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/malta"):  
            await self.highrise.send_whisper(user.id,f"{user.username} aqui esta tu malta🥤")
            await self.highrise.react("thumbs", user.id)
        if message.startswith("/vino"):  
            await self.highrise.send_whisper(user.id,f"🥃aqui está su vino {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/vodka"):  
            await self.highrise.send_whisper(user.id,f"🥃aqui está su vaso de vodka {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/whisky"):  
            await self.highrise.send_whisper(user.id,f"aqui está {user.username} su whisky🥃")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/ron"):  
            await self.highrise.send_whisper(user.id,f"{user.username} pruebe su gran ron...cortesia de la casa😄🥃")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/mal-portada"):
            await self.highrise.send_whisper(user.id,f"su botella de mal portada {user.username} 🍾")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/dragon"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su ron dragon {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/cerveza"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Cerveza {user.username} 🍺")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/coca"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Coca 🥤 {user.username} 🥤")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/rumbaron"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Botella De rumbaron {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/agua"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Botellita De Agua{user.username} 🍶🌊")
            await self.highrise.react("thumbs", user.id)
           
        if message.startswith("/anis"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Ani {user.username} 🥂")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/5-estrellas"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Vaso De Ron 5 estrellas {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/macondo"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Botella De Macondo {user.username} 🥃")
            await self.highrise.react("thumbs", user.id)
        ...
        if message.startswith("/cartilla2"):
            await self.highrise.send_whisper(user.id,"esse é o nosso cardapio de comidas e petiscos espero que goste 😄")
                                     
        if message.startswith("/cartilla2"):
            await self.highrise.send_whisper(user.id,"/pizza , /torta-de-fresa , /ensalada-de-tomate , /camaron , /fideos , /perros")

        if message.startswith("/cartilla2"):
            await self.highrise.send_whisper(user.id,"/torta , /copa , /tinita , /pan , /salchipapa , /hamburguesa , /ponquecito , /pão-de-alho")

        if message.startswith("/pizza"):  
            await self.highrise.send_whisper(user.id,f"{user.username} aqui está sua deliciosa pizza 🍕")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/torta-de-fresa"):
            await self.highrise.send_whisper(user.id,f"Aqui Está su torta de fresa {user.username} 🍰")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/ensalada-de-tomate"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Deliciosa ensalada {user.username} 🍅🧅")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/camaron"):  
            await self.highrise.send_whisper(user.id,f"🍤Aqui Está Su Delicoso Camaron 🍤 {user.username} 🍤")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/fideos"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está su fideos {user.username} 🍝")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/perros"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está a Su perro caliente {user.username} 🌭")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/torta"):  
            await self.highrise.send_whisper(user.id,f"aqui está su torta de auyama  {user.username} 🥮")
            await self.highrise.react("thumbs", user.id)
           
        if message.startswith("/copa"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Copa de helado {user.username} 🍨")
            await self.highrise.react("thumbs", user.id)
          
        if message.startswith("/tinita"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su tinita {user.username} 🍦🍨")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/pan"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su pan {user.username} 🥖")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/salchipapa"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Salchipapas {user.username}  🍟")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/hamburguesa"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su Hamburguesa especial {user.username} 🍔")
            await self.highrise.react("thumbs", user.id)

        if message.startswith("/ponquecito"):  
            await self.highrise.send_whisper(user.id,f"Aqui Está Su ponquecito {user.username} 🧁")
            await self.highrise.react("thumbs", user.id)
        ...
        if message.lower().startswith(("!cartera","cartera","/cartera","wallet","/wallet","!wallet")):
          if user.username in moderators:
            wallet = (await self.highrise.get_wallet()).content
            await self.highrise.send_whisper(user.id,f"\nvalor disponivel en cartera:\n{wallet[0].amount} {wallet[0].type}")
        ...
        if message.startswith("!ban"):
          if user.username in moderators:
              ...
              if username_to_ban.lower() in [user.username.lower() for user in users]:
                  ...
                  await self.highrise.chat(f"Bananeado  {mention} por 1 hora. ")
              else:
                  await self.highrise.send_whisper(user.id, f"usuario {mention} no esta en la sala. ")
        ...
        if message.startswith("!mute"):
          if user.username in moderators:
              ...
              if username_to_mute.lower() in [user.username.lower() for user in users]:
                  ...
                  await self.highrise.chat(f"{mention} fue muteado por 1 hora. ")
              else:
                  await self.highrise.send_whisper(user.id, f"usuario {mention} no está en la sala. ")
        ...
        if message.startswith("!unmute"):
          if user.username in moderators:
              ...
              if username_to_mute.lower() in [user.username.lower() for user in users]:
                  ...
                  await self.highrise.chat(f"{mention} fue desmuteado un pendejo.")
              else:
                  await self.highrise.send_whisper(user.id, f"usuario {mention} no está aca. ")
        ...
        if message.startswith("!kick"):
          if user.username in moderators:
              ...
              await self.highrise.chat("Tu no tienes permiso para usar ese comando abusador.")
              return
          ...
          for room_user, pos in room_users:
              if room_user.username.lower() == username.lower():
                  ...
                  break
          if "user_id" not in locals():
              await self.highrise.chat("usuario no encontrado, porfavor arregle la cordenada o codigo")
              return
          #kick user
          try:
          ...
          await self.highrise.chat(f"{username} Fue exportado de Venezuela!!")
        ...
        if message == "!fit 1" and user.username in moderators:
          ...
          await self.highrise.chat(f"👕FIT[1] Nueva ropa cambiada.") 
          

        if message == "!fit 2" and user.username in moderators:
          shirt = ["shirt-n_philippineday2019filipinotop"]
          ...
          xox = await self.highrise.set_outfit(outfit=[
                  ...
                  Item(type='clothing', amount=1, id='mouth-basic2018lollipop', account_bound=False, active_palette=24),
          ...
          await self.highrise.chat(f"👕FIT[2] Nueva ropa cambiada.")
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
          ...
          await self.highrise.chat(f"👕FIT[3] Nueva ropa cambiada.")
        ...
        if message == "!fit 4" and user.username in moderators:
          ...
          await self.highrise.chat(f"👕FIT[4] Nueva ropa cambiada.")
        ...
        if message == "!fit 5" and user.username in moderators:
          ...
          await self.highrise.chat(f"👕FIT[5] Nueva ropa cambiada.")       
        ...
        if message == "!fit 6" and user.username in moderators:
          ...
          await self.highrise.chat(f"👕FIT[6] Nueva ropa cambiada.")
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
          ...
          await self.highrise.chat(f"👕FIT[7] Nueva ropa cambiada.")
        ...
        if message == "!fit 8" and user.username in moderators:
          ...
          await self.highrise.chat(f"👕FIT[8] Nueva ropa cambiada.")
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
          ...
          await self.highrise.chat(f"👕FIT[9] Nueva ropa cambiada.")
...
Item(type='clothing', amount=1, id='eye-n_basic2018liftedeyes', account_bound=False, active_palette=7),
          ...
          await self.highrise.chat(f"👕FIT[10] Nueva ropa cambiada.")
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
          ...
          await self.highrise.chat(f"👕FIT[11] Nueva ropa cambiada.")
              ...
              if 1 <= num_hearts <= 100:
                  for _ in range(num_hearts):
                      ...
                      if target_user:
                          ...
                          await self.highrise.chat(f"o usuário {target_username} No está disponible en esta vaina. ")
              else:
                  await self.highrise.chat("cantidad inválida!, use apenas 1-100!")
          except ValueError:
        ...
        if message.lower().startswith(("!thumbs","/thumbs","-thumbs","Thumbs","thumbs")):
              ...
              if 1 <= num_hearts <= 10:
                  ...
                  await self.highrise.chat("cantidad inválida!, use apenas 1-10!\n\napenas moderadores pueden dar cantidades encima de 10!")
                  await self.highrise.chat("![react|heart|wink|wave|clap|thumbs] @username cantidad.")
              ...
              if 1 <= num_hearts <= 10:
                  for _ in range(num_hearts):
                      ...
                      if target_user:
                          ...
                          await self.highrise.chat(f"o usuário {target_username} No esta disponible en esta vaina. ")
              else:
                  await self.highrise.chat("cantidad inválida!, use apenas 1-10!\n\napenas moderadores pueden dar cantidads encima de 10")
                  await self.highrise.chat("![react|heart|wink|wave|clap|thumbs] @username cantidad.")
    ...
    async def on_message(self, user_id: str, conversation_id: str, is_new_conversation: bool) -> None:
        ...
        if message.lower().lstrip().startswith(("emotes","!emotes","-emotes","/emotes")):
            await asyncio.sleep(2)
            await self.highrise.send_message(conversation_id, f"💡 Epale sabes que puedes usar estos emotes pagos solo escribe el nombre en el chat . Ejemplo:\n\nswordfight\ngravity\nuwu\n\nPara ver todos os emotes gratuitos, use: !emote list")

        if message.lower().lstrip().startswith(("juegos","!juegos","Juegos","/juegos")):
            await asyncio.sleep(2)
            await self.highrise.send_message(conversation_id, f"🎮distraete con nuestros juegos solo escribe el nombre del juego en el chat. Ejemplo:\n\n/play\n/tarot\n/pescar\n/amor\n/odio\n/locura\n/rps [piedra|papel|tijera]\n/moneda [cara ou cruz]\n/ask [su pregunta]")

        if message.lower().lstrip().startswith(("ola","olá","oi","Olá")):
            await asyncio.sleep(2)
            await self.highrise.send_message(conversation_id, f"🤖Hola buscas ayuda utiliza !help para obtener mi lista de comandos preciosa.")
            
        if message.lower().lstrip().startswith(("!help","/help","-help","help")):
            await asyncio.sleep(2)
            await self.highrise.send_message(conversation_id, f"\n💡Que estás buscando pequeño diablillo?\n\n!mod\n!juegos\n!general")
    ...
    async def on_whisper(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username in moderators:
              ...
              try:
                  ...
                  await self.highrise.chat("💰Cantidad invalida!")
                  return
              # Checks if the bot has the amount
              bot_wallet = await self.highrise.get_wallet()
              bot_amount = bot_wallet.content[0].amount
              if bot_amount < amount:
                  await self.highrise.chat("💰No hay propina suficientes")
                  return
              # Get all users in the room
              room_users = await self.highrise.get_room_users()
              # Check if the bot has enough funds to tip all users the specified amount
              total_tip_amount = amount * len(room_users.content)
              if bot_amount < total_tip_amount:
                  await self.highrise.chat("💰No hay propina suficiente para todos! ")
                  return
              # Tip each user in the room the specified amount
              for room_user, pos in room_users.content:
                  bars_dictionary = {
                  ...
                  for bar in bars_dictionary:
                      if remaining_amount >= bar:
                          ...
                          for i in range(bar_amount):
                              ...
                              total = bar + fees_dictionary[bar]
                  if total > bot_amount:
                      await self.highrise.chat("💰No hay propina suficiente!")
                      return
                  for bar in tip:
                      await self.highrise.tip_user(room_user.id, bar)           
                      await self.highrise.chat(f"💰{room_user.username} Recibí un consejo de {amount} oro!")
                              
        if message.lower().startswith(("!cartera","cartera","/cartera","wallet","/wallet","!wallet")):
          if user.username in moderators:
            wallet = (await self.highrise.get_wallet()).content
            await self.highrise.send_whisper(user.id,f"\ncantidad disponible en mi bolsillo :\n{wallet[0].amount} {wallet[0].type}")
            await self.highrise.send_emote("idle-floss")
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
        ...
        await self.highrise.chat(f"💰{sender.username} dio a  {receiver.username} {tip.amount} oro!")
    ...
    async def on_user_leave(self, user: User) -> None:
        print(f"{user.username} se fue pal coño")
        await self.highrise.send_emote(random.choice(emote))

        await asyncio.sleep(60)
        await self.highrise.chat(f"Si necesitas ayuda preguntale a alguien o también utilice !help")